...
class TwitterClient {
  ...
  Future<List<Trend>> searchTrends([int id = 1]) async {
    String base = '/1.1/trends/place.json';
    final response = await _twitterGet(base, [
      ["id", id.toString()]
    ]);
    ...
    if (response.statusCode == 200) {
      try {
        final trends = List<Trend>();
        json
            .decode(response.body)[0]['trends']
            .forEach((data) => trends.add(Trend(data)));
        return trends;
      } catch (e) {
        print(e);
        return null;
